  ...
  addEventListener("DOMContentLoaded", () => {
    ...
    setTimeout(() => {
      ...
      canvas.getObjects().forEach(obj => {
        // Verificar si el objeto es de tipo texto
        if (obj.type === "text") {
          
          let fontWeight; // Declarar fontWeight fuera de los bloques if
          
          // Asignar el valor adecuado para fontWeight
          if (obj.fontWeight === "normal") {
            fontWeight = 300; // Asignar 300 para "normal"
          } else if (obj.fontWeight === "bold") {
            fontWeight = 700; // Asignar 700 para "bold"
          }
          ...
          // Asignar el fontWeight
          obj.fontWeight = fontWeight;
  
          // Deshabilitar la selección y los eventos
          obj.selectable = false;
          obj.evented = false;
        }
      });
      
      // Volver a renderizar el lienzo
      canvas.renderAll();
    }, 5000);
  });
